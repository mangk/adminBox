...
type record struct {
	...
	Query        url.Values  `json:"query" form:"query" gorm:"column:query;serializer:json;comment:GET参数"`
...
func TraceLogger(flags ...int) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		traceLog := log.Trace(trace)
		ctx.Set("__trace__", trace)
		...
		if !isFileUpload(ctx) &&
			...
			re.Query = ctx.Request.URL.Query()
		}
		...
		go func() {
			re.Latency = fmt.Sprintf("%v", time.Since(start))
