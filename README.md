...
- **`httpx`** 是底座 — Gin + Cobra + 服务管理，可以独立使用
...
func main() {
    ...
    httpx.Execute("my-app", "My AdminBox App")
}
```
...
func main() {
    ...
    httpx.Execute("my-admin", "My Admin Panel")
}
```
...
| 示例 | 说明 | 运行 |
|---|---|---|
| [minimal](./examples/minimal) | 仅 httpx + 自定义路由 | `go run ./examples/minimal/ run` |
...
| 包 | 说明 | 独立使用 |
|---|---|---|
| `httpx` | HTTP 服务底座 (Gin + Cobra + daemon) | ✅ |
| `admin` | 后台管理业务模块 | 需 httpx |
| `admin/front` | 嵌入式 Vue 前端界面 | 需 httpx |
| `config` | 配置管理 (Viper, YAML, 热加载) | ✅ |
| `log` | 结构化日志 (Zap, 文件轮转) | ✅ |
| `db` | 数据库 ORM (GORM, 多驱动) | ✅ |
| `cache` | 缓存（本地 + Redis） | ✅ |
| `jwt` | JWT 令牌创建与解析 | ✅ |
| `casbin` | 权限引擎 (Casbin + GORM) | ✅ |
| `middleware` | Gin 中间件集合 | ✅ |
| `response` | 统一响应格式 | ✅ |
| `request` | 请求参数解析 | ✅ |
| `upload` | 文件上传 (local/OSS/COS/S3/Qiniu) | ✅ |
| `util` | 通用工具函数 | ✅ |
