...
func SysUserMap() map[int]SysUser {
	users, _ := cache.RedisHOQ("SysUser:map", func() (data []SysUser, exp time.Duration, err error) {
		db.DB().Select("id,nick_name").Find(&data)
		exp = time.Hour * 1
		return
	})
	userMap := map[int]SysUser{}
	for _, v := range users {
		userMap[v.ID] = v
	}
	return userMap
